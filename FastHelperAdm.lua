...
-- === ПРИМЕНЕНИЕ ОБНОВЛЕНИЯ ПРИ ЗАГРУЗКЕ ===
if doesFileExist(new_path) then
    ...
    sampAddChatMessage("{00FF00}[FastHelperAdm] Обновление применено автоматически", -1)
    sampAddChatMessage("{00FF00}[FastHelperAdm] Перезагрузите скрипт (F5)", -1)
    thisScript():reload()
    return
end
...
-- ===== УЛУЧШЕННАЯ СЕКЦИЯ АВТО-ОБНОВЛЕНИЯ (UltraFuck style) =====
function checkUpdate()
    ...
    lua_thread.create(function()
        ...
        downloadUrlToFile(VERSION_URL, script_path..".version", function(id, status)
            if status ~= 58 then -- 58 = успешное скачивание
                return
            end
            ...
            -- Уведомляем о найденном обновлении
            sampAddChatMessage("{33CCFF}[FastHelperAdm] Найдено обновление v"..online, -1)
            sampAddChatMessage("{33CCFF}[FastHelperAdm] Скачиваю обновление...", -1)
            ...
            downloadUrlToFile(SCRIPT_URL, script_path..".new", function(id2, status2)
                ...
                -- Файл загружен, предлагаем перезагрузить
                sampAddChatMessage("{00FF00}[FastHelperAdm] Обновление успешно загружено!", -1)
                sampAddChatMessage("{FFFF00}[FastHelperAdm] Введите /plup для применения обновления", -1)
            end)
        end)
    end)
end

-- Команда для применения обновления
local function registerUpdateCommand()
    sampRegisterChatCommand("plup", function()
        if doesFileExist(script_path..".new") then
            ...
            sampAddChatMessage("{00FF00}[FastHelperAdm] Обновление применено! Перезагрузите скрипт (F5)", -1)
        else
            sampAddChatMessage("{FF4444}[FastHelperAdm] Нет доступных обновлений", -1)
        end
    end)
end
...
function main()
    ...
    -- Регистрируем команду обновления
    registerUpdateCommand()
    
    -- Запускаем проверку обновлений через 10 секунд
    lua_thread.create(function()
        wait(10000)
        checkUpdate()
    end)
    ...
    sampAddChatMessage("{00FF00}FastHelperAdm v"..SCRIPT_VERSION.." загружен",-1)
    sampAddChatMessage("{00FF00}Автор: Alim Akimov (@waldemar03)",-1)
    sampAddChatMessage("{00FF00}Команды: /plhelp - меню, /pl [id] [код] - быстрый ответ",-1)
    sampAddChatMessage("{00FF00}Обновление: /plup - применить обновление",-1)
    sampAddChatMessage("{FF0000}========================================",-1)

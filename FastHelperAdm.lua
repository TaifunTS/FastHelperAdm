...
function checkUpdate()
    ...
    downloadUrlToFile(VERSION_URL, versionFile, function(_, status)
        ...
        lua_thread.create(function()
            ...
            sampAddChatMessage(
                "{33CCFF}[FastHelperAdm] Íàéäåíî îáíîâëåíèå v"..online, -1
            )
            ...
            downloadUrlToFile(SCRIPT_URL, new_path, function(_, st)
                if st ~= 58 then
                    sampAddChatMessage("{FF4444}[FastHelperAdm] Îøèáêà çàãðóçêè", -1)
                    return
                end

                sampAddChatMessage(
                    "{00FF00}[FastHelperAdm] Îáíîâëåíèå çàãðóæåíî. Ïåðåçàïóñòè èãðó", -1
                )
            end)
        end)
    end)
...
function main()
    ...
    -- ?? ÎÄÈÍ ÅÄÈÍÑÒÂÅÍÍÛÉ ÂÛÇÎÂ checkUpdate
    lua_thread.create(function()
        wait(10000)
        checkUpdate()
    end)
    ...
    sampAddChatMessage("{00FF00}Âåðñèÿ: v"..SCRIPT_VERSION,-1)
    sampAddChatMessage("{FF0000}========================================",-1)

...
-- ===== ÑÅÊÖÈß ÀÂÒÎ-ÎÁÍÎÂËÅÍÈß (ÈÑÏÐÀÂËÅÍÍÀß) =====
...
function checkUpdate()
    ...
    downloadUrlToFile(VERSION_URL, tmpPath, function(_, status)
        if status ~= 58 then 
            -- Î÷èùàåì âðåìåííûé ôàéë åñëè çàãðóçêà íå óäàëàñü
        ...
        -- ?? ÔÈÊÑ ¹1: Óáèðàåì âñå ïðîáåëû è ïåðåíîñû ñòðîê ïåðåä ñðàâíåíèåì
        versionText = versionText:gsub("%s+", "")
        
        -- Ñðàâíèâàåì âåðñèè êàê ñòðîêè
        if versionText ~= CURRENT_VERSION then
            sampAddChatMessage(
                "{33CCFF}[FastHelperAdm] Íàéäåíî îáíîâëåíèå v"..versionText..
                " (ó âàñ v"..CURRENT_VERSION..")", -1
            )
            sampAddChatMessage(
                "{33CCFF}[FastHelperAdm] Íà÷èíàþ çàãðóçêó îáíîâëåíèÿ...", -1
            )
            
            -- ?? ÔÈÊÑ ¹2: Ñêà÷èâàåì âî âðåìåííûé ôàéë, à íå â òåêóùèé
            local TEMP_PATH = SCRIPT_PATH .. ".new"
            
            downloadUrlToFile(SCRIPT_URL, TEMP_PATH, function(_, st)
                if st == 58 then
                    -- Ñêà÷èâàíèå çàâåðøåíî óñïåøíî, ïðèìåíÿåì îáíîâëåíèå
                    applyUpdate(TEMP_PATH)
                else
                    sampAddChatMessage(
                        "{FF4444}[FastHelperAdm] Îøèáêà çàãðóçêè îáíîâëåíèÿ (ñòàòóñ: "..tostring(st)..")",
                        -1
                    )
                    -- Óäàëÿåì âðåìåííûé ôàéë ïðè îøèáêå
            ...
            sampAddChatMessage(
                "{00FF00}[FastHelperAdm] Ó âàñ àêòóàëüíàÿ âåðñèÿ v"..CURRENT_VERSION,
                -1
            )
        end
    end)
...
-- ?? ÔÈÊÑ ¹2: Ôóíêöèÿ ïðèìåíåíèÿ îáíîâëåíèÿ ñ èñïîëüçîâàíèåì âðåìåííîãî ôàéëà
function applyUpdate(tempPath)
    if not doesFileExist(tempPath) then
        sampAddChatMessage(
            "{FF4444}[FastHelperAdm] Îøèáêà: âðåìåííûé ôàéë îáíîâëåíèÿ íå íàéäåí",
            -1
        )
        return
    end
    
    -- Çàêðûâàåì âñå ôàéëîâûå äåñêðèïòîðû
    if io then io.flush() end
    
    -- Ïûòàåìñÿ óäàëèòü ñòàðûé ôàéë
    ...
    if not success then
        sampAddChatMessage(
            "{FF4444}[FastHelperAdm] Íå ìîãó óäàëèòü ñòàðûé ôàéë: "..tostring(err),
            -1
        )
        -- Ïûòàåìñÿ ïåðåèìåíîâàòü ïîâåðõ ñóùåñòâóþùåãî
        os.rename(tempPath, SCRIPT_PATH)
    else
        -- Ïåðåèìåíîâûâàåì âðåìåííûé ôàéë â îñíîâíîé
        os.rename(tempPath, SCRIPT_PATH)
    end
    
    -- Ïðîâåðÿåì, ÷òî ôàéë ñóùåñòâóåò
    if doesFileExist(SCRIPT_PATH) then
        sampAddChatMessage(
            "{00FF00}[FastHelperAdm] Îáíîâëåíèå óñòàíîâëåíî! Ñêðèïò ïåðåçàãðóæàåòñÿ...",
            -1
        )
        wait(1000)
        thisScript():reload()
        ...
        sampAddChatMessage(
            "{FF4444}[FastHelperAdm] Êðèòè÷åñêàÿ îøèáêà: ôàéë ñêðèïòà íå íàéäåí ïîñëå îáíîâëåíèÿ",
            -1
        )
    end
end
...
function doRazdacha()
    ...
    -- ?? ÓÄÀËÅÍÎ: Íå îòïðàâëÿåì /a z aad äëÿ ðàçäà÷è
...
function main()
    ...
    sampAddChatMessage("{00FF00}FastHelperAdm çàãðóæåí",-1)
    sampAddChatMessage("{00FF00}Àâòîð: Alim Akimov (@waldemar03)",-1)
    sampAddChatMessage("{00FF00}Âåðñèÿ: v"..CURRENT_VERSION,-1)
    sampAddChatMessage("{FF0000}========================================",-1)
    sampAddChatMessage("{ADFF2F}Äëÿ îòêðûòèÿ ìåíþ ñêðèïòà ïðîïèøèòå êîìàíäó /plhelp",-1)
    sampAddChatMessage("{ADFF2F}Äëÿ èñïîëüçîâàíèÿ ñêðèïòà ïðîïèøèòå êîìàíäó /pl [id]",-1)
    ...
    while true do
        ...
        if active_razd and active_razd2 and not antiFlood then
            ...
            if not sampIsPlayerConnected(razd_player_id) then
                sampAddChatMessage('{FF4444}[AutoRazdacha] Èãðîê âûøåë, ðàçäà÷à îòìåíåíà.',-1)
                
                -- ?? ÏÎËÍÛÉ ÑÁÐÎÑ ÔËÀÃÎÂ ïðè îòìåíå
                razdLocked = false
                antiFlood = false
                active_razd = false
                active_razd2 = false
...
function imgui.OnDrawFrame()
    ...
    elseif selectedTab == 7 then
        ...
        if adminLevel.v >= 9 then
            ...
            if otbor_selectLeader.v == 1 then
                ...
                imgui.Combo(u8"Ëèäåðêà", otbor_leader_combo, fractionNames, #fractions)
            else
                -- Åñëè âðó÷íóþ, ïîêàçûâàåì ïîëå äëÿ ââîäà
    ...
    elseif selectedTab == 9 then
        imgui.TextWrapped(u8(
            ...
            "v1.75  Èñïðàâëåíî àâòî-îáíîâëåíèå (ñòàáèëüíàÿ ðàáîòà ñ GitHub)"
        ))

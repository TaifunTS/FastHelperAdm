...
function checkUpdate()
    ...
    downloadUrlToFile(VERSION_URL, tmpPath, function(_, status)
        ...
        if onlineVersion > CURRENT_VERSION then
            sampAddChatMessage(
                "{33CCFF}[FastHelperAdm] Íàéäåíî îáíîâëåíèå v"..onlineVersion..
            ...
            downloadUrlToFile(SCRIPT_URL, SCRIPT_PATH, function(_, st)
                if st == 58 then
                    sampAddChatMessage(
                        "{00FF00}[FastHelperAdm] Îáíîâëåíèå óñòàíîâëåíî! Ïåðåçàïóñòèòå èãðó.",
                        -1
                    )
                end
            end)
...
function startAutoMP()
    ...
    -- Ñáðàñûâàåì ôëàã ïðåôèêñà
    prefixSent = false
    
    lua_thread.create(function()
        -- Îòïðàâëÿåì ïðåôèêñ òîëüêî îäèí ðàç
...
function main()
    ...
    sampAddChatMessage("{00FF00}Âåðñèÿ: v"..thisScript().version,-1)
    sampAddChatMessage("{FF0000}========================================",-1)
    ...
    while true do
        ...
        if active_razd and active_razd2 and not antiFlood then
            ...
            -- Îòïðàâëÿåì ñ ïðåôèêñîì äëÿ ðàçäà÷è (òîëüêî îäèí ðàç)
            if not prefixSent then
